''' RigidBody and internal StateChange '''
...
class StateChange:
    ...
    def multiply(self, time):
        ''' multiply components of state by time '''
        return StateChange(
            self.vel.scale(time), self.acc.scale(time),
            self.theta_vel * time, self.theta_acc * time)

    def add(self, other):
        ''' add components of another state to this one '''
        return StateChange(
            self.vel.add(other.vel), self.acc.add(other),
            self.theta_vel + other.theta_vel, self.theta_acc + other.theta_acc)
